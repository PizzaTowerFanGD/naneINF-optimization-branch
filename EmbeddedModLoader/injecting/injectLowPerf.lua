...
love.thread.getChannel(channelName):push({
    ...
    while activeThreads do -- _G.startedInjecting <= _G.finishedInjecting
        ...
        for fileName, filePatches in pairs(sortedTomls) do
            ...
            if TESTNEXT[fileName] == true then
                goto lowMemoryMode
            end
            if not next then
                goto lowMemoryMode
        ...
        -- i dont even wanna talk about this code man
        ...
        for index, thread in pairs(threads) do
            ...
            if thread[1]:isRunning() or not shouldStop then
                activeThreads = true
            end
